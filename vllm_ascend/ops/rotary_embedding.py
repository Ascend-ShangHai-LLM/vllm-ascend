...
class AscendMRotaryEmbedding(MRotaryEmbedding):
    ...
    def forward_oot(
        ...
        inv_freq_expanded = self.inv_freq[None, None, :, None].float().expand(3, positions.shape[1], -1, 1)
        position_ids_expanded = positions[:, :, None, :].float()  # shape (3, bs, 1, positions)
        device_type = query.device.type if isinstance(query.device.type, str) and query.device.type != "mps" else "cpu"
        with torch.autocast(device_type=device_type, enabled=False):  # Force float32
            freqs = (inv_freq_expanded.float().to(query.device) @ position_ids_expanded.float()).transpose(2, 3)
        ...
        query, key = apply_rotary_pos_emb_cuda_for_partial_rotary(query, key, cos, sin)
